...
`/mix/` 요청은 전체 실행 결과와 각 하위 요청의 결과를 별도로 기록합니다.
모든 기록은 동일한 `request_id`를 사용하므로 로그 목록에서 하나의 mix 실행을 추적할 수 있습니다.

- 성공한 하위 요청은 개별 기록하지 않고 전체 요약에 포함
- 하위 요청의 4xx 또는 지원하지 않는 경로: `WARNING`
- 하위 요청의 5xx 또는 처리되지 않은 예외: `ERROR`
- 전체 실행: `mix_total`, `mix_success`, `mix_failed`, `mix_skipped` 요약 필드 기록

하위 요청 실패 기록에는 `mix_key`, `mix_path`, `mix_total`이 포함되고,
처리시간은 공통 `duration_ms` 필드에 기록됩니다. 전체 요약의 `mix_results`에는
각 하위 요청의 `key`, `path`, `state`, `status`, `duration_ms`가 포함됩니다.
