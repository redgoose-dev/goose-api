...
요청 출처와 클라이언트 추적을 위해 `referer`, `origin`, `client_ip`,
`user_agent`를 요청 정보에 기록합니다. 전달된 헤더나 연결 정보가 없으면
해당 값은 `null`로 남습니다.
...
로그 스키마 마이그레이션은 지원하지 않으므로 스키마가 변경되면 기존
`log.sqlite`를 삭제하고 서비스를 다시 시작합니다.
